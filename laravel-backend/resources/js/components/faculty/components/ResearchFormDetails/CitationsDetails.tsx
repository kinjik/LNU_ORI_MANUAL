...
function CitationsDetails({ citations, status, documents, coauthors, externalAuthors }: CitationsDetailsType) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await api.post(
        `/api/research-monitoring-form/${citations.researchmonitoringform_id}/add-document`,
        formData,
      ...
      await queryClient.invalidateQueries([
        "monitoringForm",
        String(citations.researchmonitoringform_id),
      ]);
      setPendingFile(null);
      setPreviewUrl(null);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="space-y-5">
        ...
        <div>
          <h2 className="mb-2 font-semibold">Supporting Document(s):</h2>
