...
const PublishedResearchPoints = () => {
  const [data, setData] = useState<PublishedDataType>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await api.get("/api/published/points", {
  ...
  const handleUpdate = async () => {
    await api.put(`/api/published/${selectedResearch.current}/update-points`, {
      points: points,
    });
  ...
  return (
    <>
      <h1 className="text-1xl text-gray-900">
        Published Research Article/Creative Works Points
      </h1>
...
export default PublishedResearchPoints;
