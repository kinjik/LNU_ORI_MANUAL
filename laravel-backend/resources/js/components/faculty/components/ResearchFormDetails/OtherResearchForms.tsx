...
type OtherResearchFormType = {
  otherresearch: OtherResearchType;
  documents: Researchdocument[];
  status: string;
};

function OtherResearchForm({
  otherresearch,
  documents,
  status,
}: OtherResearchFormType) {
  const [openModal, setOpenModal] = useState(false);
  const navigate = useNavigate();

  const { Delete } = useMutationResearchMonitoringForm();

  const deleteForm = Delete(otherresearch.researchmonitoringform_id);
  ...
  return (
    <>
      <div className="space-y-5">
        <div>
          <h2 className="font-semibold">Research Involvement:</h2>
          <p className="pl-5 capitalize underline">
            {otherresearch.research_involvement}
          </p>
        </div>
        {otherresearch.research_title && (
          <div>
            <h2 className="font-semibold">Research Title:</h2>
            <p className="pl-5 capitalize underline">
              {otherresearch.research_title}
            </p>
          </div>
        ...
        <div>
          <h2 className="font-semibold">Nature of Fund Source:</h2>
          <p className="pl-5 capitalize underline">
            {otherresearch.fund_source_nature}
          </p>
        </div>
